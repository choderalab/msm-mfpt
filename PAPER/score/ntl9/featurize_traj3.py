...
x = sorted(glob('NTL9-3-protein-*.dcd'))
...
for i in np.arange(0,len(x),32):
    ...
    for index,j in enumerate(range(i, i+32)):
        if index < 10:
            np.save('NTL9-3-protein-00{}.npy'.format(j), contacts[index])
        elif index < 100:
            np.save('NTL9-3-protein-0{}.npy'.format(j), contacts[index])
        else:
            np.save('NTL9-3-protein-{}.npy'.format(j), contacts[index])    
